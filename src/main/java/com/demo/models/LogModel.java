...
public class LogModel {
	public List<Log> findAll() {
		...
		try {
			PreparedStatement preparedStatement = ConnectDB.connection().prepareStatement("select * from logs");
	...
	public boolean create(Log log) {
		...
		try {
			PreparedStatement preparedStatement = ConnectDB.connection()
					.prepareStatement("INSERT INTO logs(ip, level, national, time, beforeValue, afterValue) VALUES (?,?,?,?,?,?)");
